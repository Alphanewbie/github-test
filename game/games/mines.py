...
class MineBoard:
    ...
    def __init__(self, boardsize=10, bombnum=10, dic = None):
        if dic == None:
            # 보드의 크기
            ...
            self.boardsize = dic.get('boardsize')
            # 지뢰가 설정되어 있는 테이블
            self.board = dic.get('board')
            # 게임을 할 테이블
            self.findedbrd = dic.get('findedbrd')
            # 폭탄 리스트
            self.bombpos = dic.get('bombpos')
            # 깃발 리스트
            self.flag = dic.get('flag')
    ...
    def search_board(self, y, x):
        answer = 0
        if self.findedbrd[y][x] != 'A' and self.findedbrd[y][x] != 'F':
